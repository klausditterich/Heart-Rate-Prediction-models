...
class Fourier(nn.Module):
    def __init__(self, input_size, hidden_size, device):
        ...
        self.filters = nn.Parameter(torch.tensor(np.hamming(400), dtype=torch.float32).unsqueeze(0))

        self.fc = nn.Linear(hidden_size, 800, bias=False)

        self.fc2 = nn.Linear(hidden_size, 800, bias=False)

        self.fc3 = nn.Linear(200, 147)
        ...
        self.fc3.weight.data = torch.randn(200, 147).transpose(1, 0)
        ...
        self.filters.data = torch.load("C:/Users/Usuario/Downloads/hamming_weights_BAMI.pth")
    ...
    def forward(self, x):
        ...
        psd = torch.abs(fft * fftconj) / (400 ** 2)

        freq_probs = self.fc3(psd[:, :200])
...
class CORNET(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.conv1 = nn.Conv1d(input_size, 32, 60)
        self.relu1 = nn.ReLU()
        self.batch1 = nn.BatchNorm1d(32)
        self.max1 = nn.MaxPool1d(2)
        self.drop1 = nn.Dropout(0.1)
        self.conv2 = nn.Conv1d(32, 32, 60)
        self.relu2 = nn.ReLU()
        self.batch2 = nn.BatchNorm1d(32)
        self.max2 = nn.MaxPool1d(2)
        self.drop2 = nn.Dropout(0.1)
        self.lstm1 = nn.LSTM(55, 128, 2, batch_first=True)
        self.tanh1 = nn.Tanh()
        self.fc = nn.Linear(4096, 1)

    def zero_hidden(self, device):
        self.h0_1 = torch.zeros(2, 128, 128).to(device)
        self.c0_1 = torch.zeros(2, 128, 128).to(device)
...
class CORNET2(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.tcn = TemporalConvNet(55, [128, 128, 128, 128, 128, 128], 3, 0.1, False)
        self.tanh1 = nn.Tanh()
        self.fc = nn.Linear(4096, 1)
